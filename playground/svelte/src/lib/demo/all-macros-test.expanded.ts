...
export namespace SvelteAllMacrosTest {
    export function equals(self: SvelteAllMacrosTest, other: SvelteAllMacrosTest): boolean {
...
export namespace SvelteAllMacrosTest {
    ...
    export function toObject(self: SvelteAllMacrosTest): Record<string, unknown> {
        const ctx = SerializeContext.create();
                ...
                return Result.err([
                    {
                        field: '_root',
                        message:
                            'SvelteAllMacrosTest.fromObject: root cannot be a forward reference'
                    }
                ]);
            }
            ctx.applyPatches();
    ...
    export function __deserialize(
        ...
        if (typeof value !== 'object' || value === null || Array.isArray(value)) {
            throw new DeserializeError([
                { field: '_root', message: 'SvelteAllMacrosTest.__deserialize: expected an object' }
            ]);
        }
...
export namespace SvelteAllMacrosTest {
    export function hashCode(self: SvelteAllMacrosTest): number {
        ...
        hash =
            (hash * 31 +
                (self.id ?? '').split('').reduce((h, c) => (h * 31 + c.charCodeAt(0)) | 0, 0)) |
            0;
        hash =
            (hash * 31 +
                (self.title ?? '').split('').reduce((h, c) => (h * 31 + c.charCodeAt(0)) | 0, 0)) |
            0;
        hash =
            (hash * 31 +
                (self.content ?? '')
        ...
        hash =
            (hash * 31 +
                (self.apiKey ?? '').split('').reduce((h, c) => (h * 31 + c.charCodeAt(0)) | 0, 0)) |
            0;
        hash =
            (hash * 31 +
