...
function createEnv(files, expandFn = null) {
  ...
  const host = {
    ...
    resolveModuleNames: (names) => names.map(n => {
      if (n === './macros') return { resolvedFileName: '/virtual/macros.ts', extension: '.ts' };
      return undefined;
    }),
  };
...
u.BAD_METHOD();`;
    ...
    const env = createEnv({
      ...
      sourceMapping: {
        ...
        generatedRegions: [
          { start: insertionPoint, end: insertionPoint + insertionLength, sourceMacro: 'Debug::toString' }
        ]
      }
    }));
    ...
    const tsError = diagnostics.find(d =>
      typeof d.messageText === 'string' && d.messageText.includes('BAD_METHOD')
    );
    ...
    assert.strictEqual(tsError.start, originalPos.offset,
...
class User {
  ...
  toString(): string { return this.id; }
    ...
    const quickInfo = env.pluginService.getQuickInfoAtPosition('/virtual/test.ts', expandedNamePos.offset);

    // This should work because the field exists in both
    if (quickInfo) {
      const display = quickInfo.displayParts?.map(p => p.text).join('') || '';
      console.log(`  Hover at expanded position ${expandedNamePos.offset}: ${display}`);
    ...
    const quickInfoOriginal = env.pluginService.getQuickInfoAtPosition('/virtual/test.ts', originalNamePos.offset);

    if (quickInfoOriginal) {
      const display = quickInfoOriginal.displayParts?.map(p => p.text).join('') || '';
      console.log(`  Hover at original position ${originalNamePos.offset}: ${display}`);
    } else {
      console.log(`  [WARN] No hover info at original position ${originalNamePos.offset}`);
    }
  });
...
function helper() {
  ...
  return x;
    ...
    console.log(`  Position shift: ${expandedHelperPos.offset - originalHelperPos.offset}`);

    // Get hover at original position (what IDE would send)
    const quickInfo = env.pluginService.getQuickInfoAtPosition('/virtual/test.ts', originalHelperPos.offset);
    ...
    if (quickInfo) {
      ...
      assert(display.includes('helper'),
        `Hover at original position should find 'helper', got: ${display}`);
    } else {
      assert.fail('Hover at original position returned nothing - position mapping needed');
    }
  });
...
test.describe('STRICT: Completions Position Validation', () => {
  ...
  test('completions at original position after expansion should work', (t) => {
    ...
    if (completions?.entries) {
      ...
      console.log(`  Completions at original pos: ${names.slice(0, 5).join(', ')}`);
      ...
      assert.fail('Completions failed at original position - position mapping needed');
    }
  });
